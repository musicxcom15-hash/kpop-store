-- =====================================================================
-- KPOP STORE — โครงสร้างฐานข้อมูลและข้อมูลตั้งต้น
--
-- นำเข้าผ่าน phpMyAdmin ได้เลย (มี CREATE DATABASE ให้แล้ว ไม่ต้องสร้าง DB ก่อน)
-- ทดสอบกับ MySQL 5.7.24 บน MAMP (127.0.0.1:3306)
--
-- ข้อมูลตั้งต้นถอดมาจาก DEFAULT_* ใน preorderkpopweb/js/store.js
-- เพื่อให้เปิดเว็บครั้งแรกแล้วเห็นหน้าตาเหมือนตอนใช้ localStorage ทุกประการ
--
-- หมายเหตุเรื่อง MySQL 5.7 (ไม่ใช่ 8.x):
--   * CHECK constraint ถูก parse แต่ไม่บังคับใช้ จึงใช้ ENUM แทนทุกจุดที่ต้องจำกัดค่า
--   * sql_mode มี STRICT_TRANS_TABLES + NO_ZERO_DATE คอลัมน์วันที่ที่ไม่บังคับจึงเป็น NULL
--   * character_set_server เป็น utf8 ไม่ใช่ utf8mb4 จึงต้องระบุ charset เองทุกที่
-- =====================================================================

CREATE DATABASE IF NOT EXISTS `kpop_store`
